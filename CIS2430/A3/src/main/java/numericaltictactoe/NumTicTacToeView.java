...
public class NumTicTacToeView extends JPanel {
    ...
    */
    public NumTicTacToeView(int wide, int tall, GameUI gameFrame) {
        super();
        ...
        this.play = new NumTicTacToe(3,3);
    ...
    private JLabel makeTitle() {
        JLabel title = new JLabel("Welcome to Numerical TicTacToe!");
        return title;
    }
    ...
    */
    protected void doSaveGame() {
        ...
        int choice = fileChoice.showOpenDialog(this);

        if(choice==JFileChooser.APPROVE_OPTION) {
            userFile = fileChoice.getSelectedFile();
            userFilePath = userFile.getAbsolutePath();
            userFileName = userFile.getName();
            try {
                SaveLoadFiles.save(play, userFileName, userFilePath);
            } catch(IllegalArgumentException e) {
